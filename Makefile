...
# Memory C files
memory_c_source_files := $(shell find CMOS/Memory -name '*.c')
memory_c_object_files := $(patsubst CMOS/Memory/%.c, build/memory/%.o, $(memory_c_source_files))
...
build/memory/%.o: CMOS/Memory/%.c
	mkdir -p $(dir $@)
	x86_64-elf-gcc -c -I src/ -I CMOS -I CMOS/Core -I CMOS/HAL -ffreestanding $< -o $@

.PHONY: build-x86_64
build-x86_64: $(kernel_c_object_files) $(core_c_object_files) $(x86_64_object_files) $(drivers_c_object_files) $(src_object_files) $(memory_c_object_files)
	mkdir -p dist/x86_64
	x86_64-elf-ld -n -o dist/x86_64/kernel.bin -T targets/x86_64/linker.ld \
		$(kernel_c_object_files) $(x86_64_object_files) $(drivers_c_object_files) $(src_object_files) $(core_c_object_files) $(memory_c_object_files)
